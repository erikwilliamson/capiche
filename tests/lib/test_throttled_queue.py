...
from capiche.lib import ThrottledQueue

def simple_callback(response: Response) -> str:
...
class TestThrottledQueue:
    def test_instantiation(self):
        tq = ThrottledQueue(
            max_rate=5, window=5, cache_age=30, callback=simple_callback, max_queue_size=2
        )
        assert repr(tq) == "ThrottledQueue(max_rate=5, window=5, callback=simple_callback, max_queue_size=2, cache_age=30"


    def test_queueing_request(self):
        tq = ThrottledQueue(
            max_rate=1, window=1, cache_age=30, callback=simple_callback, max_queue_size=None
        )
    ...
    def test_is_full(self):
        tq = ThrottledQueue(
            max_rate=1, window=1, cache_age=30, callback=simple_callback, max_queue_size=2
        )
    ...
    def test_rate(self):
        ...
        time.sleep(.1)  # Give it a sec to complete...
...
class TestCache:
    def test_cache_no_args(self):
        tq = ThrottledQueue(
            max_rate=10, window=1, cache_age=30, callback=simple_callback, max_queue_size=2
        )
    ...
    def test_cache_no_args_with_cache_hit(self):
        tq = ThrottledQueue(
            max_rate=10, window=30, cache_age=30, callback=simple_callback, max_queue_size=2
        )
